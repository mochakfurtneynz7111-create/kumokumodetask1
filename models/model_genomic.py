...
class SNN(nn.Module):
    def __init__(self, omic_input_dim: int, model_size_omic: str='small', n_classes: int=4):
        ...
        # self.size_dict_omic = {'small': [64, 64, 64, 64], 'big': [1024, 1024, 1024, 256]}
...
class SNN(nn.Module):
    def __init__(self, omic_input_dim: int, model_size_omic: str='small', n_classes: int=4):
        ...
        # 直接创建分类层，输入维度是omic_input_dim，输出维度是n_classes
        self.classifier = nn.Linear(omic_input_dim, n_classes)
        init_max_weights(self)  # 如果有权重初始化，保留


    def forward(self, **kwargs):
        x = kwargs['x_omic']
        h = self.classifier(x)  # 直接输入到分类层
        assert len(h.shape) == 2 and h.shape[1] == self.n_classes
        return h
